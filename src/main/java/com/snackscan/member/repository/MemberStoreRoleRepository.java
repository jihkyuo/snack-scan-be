...
public interface MemberStoreRoleRepository extends JpaRepository<MemberStoreRole, Long> {
    ...
    // 특정 역할의 멤버들이 소속된 스토어 조회
    @Query("SELECT msr FROM MemberStoreRole msr WHERE msr.member.role = :role")
    List<MemberStoreRole> findByMemberRole(@Param("role") String role);
}
